...
async def listen_for_speech(silence_timeout: float = 2.0) -> str:
    """
    Opens the microphone, streams audio to Sarvam AI, and waits for the user to finish speaking.
    Returns the final transcript string.
    
    :param silence_timeout: The amount of silence (in seconds) to wait before finalizing the transcript.
    """
    ...
    with sd.InputStream(
        ...
        with client.speech_to_text_streaming.connect(
            ...
            # Exiting the 'with' block closes the websocket
            # This automatically unblocks and terminates the sender/receiver threads.
