...
function App() {
  ...
  const startLiveMode = async () => {
    try {
      ...
      const socket = new WebSocket('ws://10.35.102.237:8000/api/live/ws');
      socket.onopen = () => {
  ...
  const connectAudioStreamWs = (token, sid) => {
    ...
    const socket = new WebSocket('ws://10.35.102.237:8000/api/audio_stream/ws');
    socket.onopen = () => {
  ...
  const fetchSessions = async () => {
    ...
    try {
      const res = await fetch('http://10.35.102.237:8000/api/sessions', {
  ...
  const handleDeleteSession = async (sessionId, e) => {
    ...
    if (!session?.access_token) return;
    try {
      await fetch(`http://10.35.102.237:8000/api/sessions/${sessionId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${session.access_token}` }
      });
      fetchSessions();
  ...
  const loadSessionHistory = async (sessionId) => {
    ...
    if (!session?.access_token) return;
    try {
      const res = await fetch(`http://10.35.102.237:8000/api/sessions/${sessionId}/history`, {
        headers: { 'Authorization': `Bearer ${session.access_token}` }
      });
      const data = await res.json();
      if (data.history) {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      const response = await fetch('http://10.35.102.237:8000/api/upload', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      const response = await fetch('http://10.35.102.237:8000/api/vision', {
        method: 'POST',
        body: formData
      });
      const data = await response.json();
  ...
  const handleGenerateImage = async (prompt) => {
    ...
    try {
      const response = await fetch('http://10.35.102.237:8000/api/generate_image', {
        method: "POST",
  ...
  const handleSendText = async () => {
    ...
    try {
      const response = await fetch('http://10.35.102.237:8000/api/chat', {
        method: 'POST',
        headers: { 
  ...
  const sendAudioMessage = async (audioBlob) => {
    ...
    try {
      const response = await fetch('http://10.35.102.237:8000/api/chat/audio', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${session?.access_token}` },
        body: formData
      });
